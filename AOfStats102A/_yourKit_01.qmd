...
```{r}
read_xlsx("./data/20201228perceptionOfAttractivenessRawData.xlsx")

# Idea, we could create a plot that compares the age, race, sex to the average beauty score to measure biases
```

2.  One table of some sort (could be simple counts could be more complex like model output)

```{r}
# Idea: we could fit a model to see which of these features(age, race sex) actually have an effect on the beauty score and which categories are most sensitive 
```
...
-   **Video attatched with submission**\*
